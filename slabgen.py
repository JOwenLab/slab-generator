...
def to_qe(slab, vacuum, comment, fix_bottom_layers=2, symmetric=False):
    ...
    - CELL_PARAMETERS / ATOMIC_POSITIONS in angstrom, ibrav=0
    - bottom `fix_bottom_layers` carbon layers (plus any H below them)
      frozen via if_pos flags for asymmetric slabs; 0 for symmetric
    - k-mesh scaled to the in-plane cell (~24 A^-1 density), 1 along z
    - PAW PBE pseudopotential placeholders (pslibrary names): EDIT
      ATOMIC_SPECIES and pseudo_dir for your local pseudo set
    - Marzari-Vanderbilt smearing by default so bare/graphitic/metallic
      surfaces converge; for clearly insulating terminations you may
      switch to occupations='fixed'
    - dipole correction lines included (commented) for asymmetric slabs
    """
    cell, pos = _cell3d(slab, vacuum)
    pos = pos.copy()
    pos[abs(pos) < 1e-8] = 0.0            # avoid printing -0.00000000
    ...
    if symmetric:
        fix_bottom_layers = 0
    # z threshold for frozen region: bottom N carbon levels
    ...
    if fix_bottom_layers > 0:
        L += ["  ! asymmetric slab: consider the dipole correction:",
              "  ! tefield = .true., dipfield = .true.  (these go in CONTROL)",
              "  ! edir = 3, emaxpos = 0.95, eopreg = 0.05, eamp = 0.0"]
    L += ["/",
          "&ELECTRONS",
          "  conv_thr    = 1.0d-8",
          "  mixing_beta = 0.3",
